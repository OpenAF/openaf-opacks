...
(function () {
  ...
  function createAccessibleText(label, status) {
    var labelPrefix = label ? `${label}: ` : ''
    return labelPrefix + status
  }

  function calcWidth(text) {
    var charWidthTable = widthsVerdana110

    var total = 0
    var fall = 70
    // iterate by Unicode code points so surrogate pairs count as one character
    for (var ch of text) {
      var cp = ch.codePointAt(0)
      var charWidth = charWidthTable[cp]
      total += isUnDef(charWidth) ? fall : charWidth
    }
    return total
  }

  function bare(aBadgenOptions) {
    aBadgenOptions = _$(aBadgenOptions, "aBadgenOptions").isMap().default({})
    _$(aBadgenOptions.status, "status").isString().$_()

    aBadgenOptions.color = colorPresets[aBadgenOptions.color] || aBadgenOptions.color || colorPresets.blue
    // New optional customizations
    aBadgenOptions.textColor = colorPresets[aBadgenOptions.textColor] || aBadgenOptions.textColor || 'fff'
    aBadgenOptions.transparent = aBadgenOptions.transparent === true
    aBadgenOptions.borderColor = colorPresets[aBadgenOptions.borderColor] || aBadgenOptions.borderColor || aBadgenOptions.color
    aBadgenOptions.shadow = aBadgenOptions.shadow !== false
    aBadgenOptions.borderStyle = aBadgenOptions.borderStyle || 'solid'

    var stTextWidth = calcWidth(aBadgenOptions.status)
    var stRectWidth = stTextWidth + 115

    aBadgenOptions.status = sanitize(aBadgenOptions.status)
    aBadgenOptions.color = sanitize(aBadgenOptions.color)
    aBadgenOptions.textColor = sanitize(aBadgenOptions.textColor)
    aBadgenOptions.borderColor = sanitize(aBadgenOptions.borderColor)
    // Prepare shadow and border dash logic
    var shadowOpacity = aBadgenOptions.style === 'classic' ? 0.25 : 0.1
    var statusShadowTag = ''
    if (aBadgenOptions.shadow) {
      statusShadowTag = '<text x="65" y="148" textLength="' + stTextWidth + '" fill="#000" opacity="' + shadowOpacity + '">' + aBadgenOptions.status + '</text>'
    }
    var borderDashAttr = ''
    if (aBadgenOptions.borderStyle && aBadgenOptions.borderStyle !== 'solid') {
      var dashMap = { dash: '24 24', dot: '6 18', dashdot: '24 18 6 18' }
      var pat = dashMap[aBadgenOptions.borderStyle]
      if (!pat) {
        // Sanitize custom pattern (digits space comma)
        pat = (aBadgenOptions.borderStyle + '').replace(/[^0-9,\s]/g, '').replace(/,+/g, ' ').trim()
        if (pat === '') pat = __
      }
      if (pat) borderDashAttr = 'stroke-dasharray="' + pat + '"'
    }

    if (aBadgenOptions.style === 'flat') {
      if (aBadgenOptions.transparent) {
        var stRectWidthPlus20 = stRectWidth + 20
        return templify('<svg width="{{svgWidth}}" height="{{svgHeight}}" viewBox="-10 -10 {{stRectWidthPlus20}} 220" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="${status}"><title>{{status}}</title><g><rect fill="none" stroke="#{{borderColor}}" stroke-width="12" {{borderDashAttr}} rx="30" x="0" width="{{stRectWidth}}" height="200"/></g><g aria-hidden="true" text-anchor="start" font-family="Verdana,DejaVu Sans,sans-serif" font-size="110">{{{statusShadowTag}}}<text x="55" y="138" textLength="{{stTextWidth}}" fill="#{{textColor}}">{{status}}</text></g></svg>', {
          svgWidth: aBadgenOptions.scale * stRectWidthPlus20 / 10,
          svgHeight: aBadgenOptions.scale * 22,
          stRectWidth: stRectWidth,
          stRectWidthPlus20: stRectWidthPlus20,
          stTextWidth: stTextWidth,
          status: aBadgenOptions.status,
          borderColor: aBadgenOptions.borderColor,
          textColor: aBadgenOptions.textColor,
          statusShadowTag: statusShadowTag,
          borderDashAttr: borderDashAttr
        })
      }
      var stRectWidthPlus20 = stRectWidth + 20
      return templify('<svg width="{{svgWidth}}" height="{{svgHeight}}" viewBox="-10 -10 {{stRectWidthPlus20}} 220" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="${status}"><title>{{status}}</title><g><rect fill="#{{color}}" x="0" width="{{stRectWidth}}" height="200" rx="30"/></g><g aria-hidden="true" fill="#fff" text-anchor="start" font-family="Verdana,DejaVu Sans,sans-serif" font-size="110"><text x="55" y="138" textLength="{{stTextWidth}}" fill="#{{textColor}}">{{status}}</text></g></svg>', {
        svgWidth: aBadgenOptions.scale * stRectWidthPlus20 / 10,
        svgHeight: aBadgenOptions.scale * 22,
        stRectWidth: stRectWidth,
        stRectWidthPlus20: stRectWidthPlus20,
        stTextWidth: stTextWidth,
        status: aBadgenOptions.status,
        color: aBadgenOptions.color,
        textColor: aBadgenOptions.textColor
      })
    }
    if (aBadgenOptions.style === 'rounded') {
      if (aBadgenOptions.transparent) {
        var stRectWidthPlus20 = stRectWidth + 20
        return templify('<svg width="{{svgWidth}}" height="{{svgHeight}}" viewBox="-10 -10 {{stRectWidthPlus20}} 220" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="${status}"><title>{{status}}</title><g><rect fill="none" stroke="#{{borderColor}}" stroke-width="12" {{borderDashAttr}} rx="100" width="{{stRectWidth}}" height="200"/></g><g aria-hidden="true" text-anchor="start" font-family="Verdana,DejaVu Sans,sans-serif" font-size="110">{{{statusShadowTag}}}<text x="55" y="138" textLength="{{stTextWidth}}" fill="#{{textColor}}">{{status}}</text></g></svg>', {
          svgWidth: aBadgenOptions.scale * stRectWidthPlus20 / 10,
          svgHeight: aBadgenOptions.scale * 22,
          stRectWidth: stRectWidth,
          stRectWidthPlus20: stRectWidthPlus20,
      ...
      var stRectWidthPlus20 = stRectWidth + 20
      return templify('<svg width="{{svgWidth}}" height="{{svgHeight}}" viewBox="-10 -10 {{stRectWidthPlus20}} 220" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="${status}"><title>{{status}}</title><g><rect fill="#{{color}}" rx="100" width="{{stRectWidth}}" height="200"/><rect fill="none" stroke="#{{borderColor}}" stroke-width="12" {{borderDashAttr}} rx="100" width="{{stRectWidth}}" height="200"/></g><g aria-hidden="true" fill="#fff" text-anchor="start" font-family="Verdana,DejaVu Sans,sans-serif" font-size="110">{{{statusShadowTag}}}<text x="55" y="138" textLength="{{stTextWidth}}" fill="#{{textColor}}">{{status}}</text></g></svg>', {
        svgWidth: aBadgenOptions.scale * stRectWidthPlus20 / 10,
        svgHeight: aBadgenOptions.scale * 22,
        stRectWidth: stRectWidth,
        stRectWidthPlus20: stRectWidthPlus20,
    ...
    if (aBadgenOptions.transparent) {
      var stRectWidthPlus20 = stRectWidth + 20
      return templify('<svg width="{{svgWidth}}" height="{{svgHeight}}" viewBox="-10 -10 {{stRectWidthPlus20}} 220" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="{{status}}"><title>{{status}}</title><g><rect width="{{stRectWidth}}" height="200" rx="30" fill="none" stroke="#{{borderColor}}" stroke-width="12" {{borderDashAttr}}/></g><g aria-hidden="true" text-anchor="start" font-family="Verdana,DejaVu Sans,sans-serif" font-size="110">{{{statusShadowTag}}}<text x="55" y="138" textLength="{{stTextWidth}}" fill="#{{textColor}}">{{status}}</text></g></svg>', {
        svgWidth: aBadgenOptions.scale * stRectWidthPlus20 / 10,
        svgHeight: aBadgenOptions.scale * 22,
        stRectWidth: stRectWidth,
        stRectWidthPlus20: stRectWidthPlus20,
    ...
    var stRectWidthPlus20 = stRectWidth + 20
    return templify('<svg width="{{svgWidth}}" height="{{svgHeight}}" viewBox="-10 -10 {{stRectWidthPlus20}} 220" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="{{status}}"><title>{{status}}</title><linearGradient id="a" x2="0" y2="100%"><stop offset="0" stop-opacity=".1" stop-color="#EEE"/><stop offset="1" stop-opacity=".1"/></linearGradient><mask id="m"><rect width="{{stRectWidth}}" height="200" rx="30" fill="#FFF"/></mask><g mask="url(#m)"><rect width="{{stRectWidth}}" height="200" fill="#{{color}}" x="0" rx="30"/><rect width="{{stRectWidth}}" height="200" fill="url(#a)" rx="30"/></g><g aria-hidden="true" fill="#fff" text-anchor="start" font-family="Verdana,DejaVu Sans,sans-serif" font-size="110">{{{statusShadowTag}}}<text x="55" y="138" textLength="{{stTextWidth}}" fill="#{{textColor}}">{{status}}</text></g></svg>', {
      svgWidth: aBadgenOptions.scale * stRectWidthPlus20 / 10,
      svgHeight: aBadgenOptions.scale * 22,
      stRectWidth: stRectWidth,
      stRectWidthPlus20: stRectWidthPlus20,
  ...
  /**
   * <odoc>
   * <key>badgen.fromSVG(aFileOrString) : String</key>
   * Converts the provided aFileOrString SVG to a base64 representation prefixed with the necessary arguments to embeed into an "img" tag in HTML
   * </odoc>
   */
  exports.fromSVG = function (aFile) {
    ...
    if (io.fileExists(path + "/svgs/" + aFile)) {
      data = io.readFileString(path + "/svgs/" + aFile)
    }
    if (io.fileExists(aFile)) {
      data = io.readFileString(aFile)
    }

    if (data == "") throw "File '" + aFile + "' not found!"

    return "data:image/svg+xml;base64," + af.fromBytes2String(af.toBase64Bytes(data))
  }

  /**
   * <odoc>
   * <key>badgen.badgen(aOptions) : String</key>
   * Given a set of aOptions in the provided map returns the necessary SVG code to draw a badge with the provided label and status. Options available:\
  ...
  *   labelTextColor (String) The color name or RGB for the label text (optional, defaults to textColor)\
   * \
   * </odoc>
   */
  exports.badgen = function (aOptions) {
    var dOptions = {
      label: "label",
      status: "status",
      color: 'blue',
    ...
    aOptions = merge(dOptions, aOptions)
    _$(aOptions.status, "status").isString().$_()

    if (!aOptions.label && !aOptions.icon) {
      return bare({ status: aOptions.status, color: aOptions.color, style: aOptions.style, scale: aOptions.scale, textColor: aOptions.textColor, transparent: aOptions.transparent, borderColor: aOptions.borderColor, shadow: aOptions.shadow, borderStyle: aOptions.borderStyle })
    }

    aOptions.color = colorPresets[aOptions.color] || aOptions.color
    aOptions.labelColor = colorPresets[aOptions.labelColor] || aOptions.labelColor
    aOptions.textColor = colorPresets[aOptions.textColor] || aOptions.textColor
    aOptions.labelTextColor = colorPresets[aOptions.labelTextColor] || aOptions.labelTextColor || aOptions.textColor
    aOptions.borderColor = colorPresets[aOptions.borderColor] || aOptions.borderColor || aOptions.color
    aOptions.iconWidth = aOptions.iconWidth * 10

    var iconSpanWidth = aOptions.icon ? (aOptions.label.length ? aOptions.iconWidth + 30 : aOptions.iconWidth - 18) : 0
    var sbTextStart = aOptions.icon ? (iconSpanWidth + 50) : 75
    var sbTextWidth = aOptions.label ? calcWidth(aOptions.label) : 0
    var stTextWidth = calcWidth(aOptions.status)
    var sbRectWidth = sbTextWidth + 115 + iconSpanWidth
    var stRectWidth = stTextWidth + 100
    var width = sbRectWidth + stRectWidth
    var xlink = aOptions.icon ? ' xmlns:xlink="http://www.w3.org/1999/xlink"' : ''

    aOptions.label = aOptions.label ? sanitize(aOptions.label) : ''
    aOptions.status = sanitize(aOptions.status)
    aOptions.color = sanitize(aOptions.color)
    aOptions.labelColor = sanitize(aOptions.labelColor)
    aOptions.textColor = sanitize(aOptions.textColor)
    aOptions.labelTextColor = sanitize(aOptions.labelTextColor)
    aOptions.borderColor = sanitize(aOptions.borderColor)
    aOptions.icon = aOptions.icon ? sanitize(aOptions.icon) : aOptions.icon
    var accessibleText = createAccessibleText(aOptions.label, aOptions.status)
    // Shadow & dash logic
    var shadowOpacity = aOptions.style === 'classic' ? 0.25 : 0.1
    var labelShadowTag = ''
    var statusShadowTag = ''
    if (aOptions.shadow) {
      if (aOptions.label) labelShadowTag = '<text x="' + (sbTextStart + 10) + '" y="148" textLength="' + sbTextWidth + '" fill="#000" opacity="' + shadowOpacity + '">' + aOptions.label + '</text>'
      statusShadowTag = '<text x="' + (sbRectWidth + 55) + '" y="148" textLength="' + stTextWidth + '" fill="#000" opacity="' + shadowOpacity + '">' + aOptions.status + '</text>'
    }
    var borderDashAttr = ''
    if (aOptions.borderStyle && aOptions.borderStyle !== 'solid') {
      var dashMap = { dash: '24 24', dot: '6 18', dashdot: '24 18 6 18' }
      var pat = dashMap[aOptions.borderStyle]
      if (!pat) {
        pat = (aOptions.borderStyle + '').replace(/[^0-9,\s]/g, '').replace(/,+/g, ' ').trim()
        if (pat === '') pat = __
      }
      if (pat) borderDashAttr = 'stroke-dasharray="' + pat + '"'
    }

    if (aOptions.style === 'flat') {
      if (aOptions.transparent) {
        var widthPlus20 = width + 20
        return templify('<svg width="{{svgWidth}}" height="{{svgHeight}}" viewBox="-10 -10 {{widthPlus20}} 220" xmlns="http://www.w3.org/2000/svg"{{{xlink}}} role="img" aria-label="{{accessibleText}}"><title>{{{accessibleText}}}</title><g><rect fill="none" stroke="#{{borderColor}}" stroke-width="12" {{borderDashAttr}} rx="30" width="{{width}}" height="200"/></g><g aria-hidden="true" text-anchor="start" font-family="Verdana,DejaVu Sans,sans-serif" font-size="110">{{{labelShadowTag}}}<text x="{{sbTextStart}}" y="138" textLength="{{sbTextWidth}}" fill="#{{labelTextColor}}">{{{label}}}</text>{{{statusShadowTag}}}<text x="{{sbRectWidth45}}" y="138" textLength="{{stTextWidth}}" fill="#{{textColor}}">{{{status}}}</text></g>{{{icon}}}</svg>', {
          svgWidth: aOptions.scale * widthPlus20 / 10,
          svgHeight: aOptions.scale * 22,
          width: width,
          widthPlus20: widthPlus20,
          xlink: xlink,
          accessibleText: accessibleText,
          borderColor: aOptions.borderColor,
          sbRectWidth: sbRectWidth,
          color: aOptions.color,
          stRectWidth: stRectWidth,
          sbTextStart10: sbTextStart + 10,
          sbTextWidth: sbTextWidth,
          label: aOptions.label,
          sbTextStart: sbTextStart,
          sbRectWidth55: sbRectWidth + 55,
          stTextWidth: stTextWidth,
          sbRectWidth45: sbRectWidth + 45,
          status: aOptions.status,
          icon: aOptions.icon ? '<image x="30" y="10" width="' + aOptions.iconWidth + '" height="182" xlink:href="' + aOptions.icon + '"/>' : "",
          textColor: aOptions.textColor,
          labelTextColor: aOptions.labelTextColor,
          labelShadowTag: labelShadowTag,
          statusShadowTag: statusShadowTag,
          borderDashAttr: borderDashAttr
        })
      }
      var widthPlus20 = width + 20
      return templify('<svg width="{{svgWidth}}" height="{{svgHeight}}" viewBox="-10 -10 {{widthPlus20}} 220" xmlns="http://www.w3.org/2000/svg"{{{xlink}}} role="img" aria-label="{{accessibleText}}"><title>{{{accessibleText}}}</title><mask id="mf"><rect width="{{width}}" height="200" rx="30" fill="#FFF"/></mask><g mask="url(#mf)"><rect fill="#{{labelColor}}" width="{{sbRectWidth}}" height="200"/><rect fill="#{{color}}" x="{{sbRectWidth}}" width="{{stRectWidth}}" height="200"/></g><g aria-hidden="true" fill="#fff" text-anchor="start" font-family="Verdana,DejaVu Sans,sans-serif" font-size="110"><text x="{{sbTextStart}}" y="138" textLength="{{sbTextWidth}}" fill="#{{labelTextColor}}">{{{label}}}</text><text x="{{sbRectWidth45}}" y="138" textLength="{{stTextWidth}}" fill="#{{textColor}}">{{{status}}}</text></g>{{{icon}}}</svg>', {
        svgWidth: aOptions.scale * widthPlus20 / 10,
        svgHeight: aOptions.scale * 22,
        width: width,
        widthPlus20: widthPlus20,
        xlink: xlink,
        accessibleText: accessibleText,
        labelColor: aOptions.labelColor,
        sbRectWidth: sbRectWidth,
        color: aOptions.color,
        stRectWidth: stRectWidth,
        sbTextStart10: sbTextStart + 10,
    ...
    if (aOptions.style === 'rounded') {
      if (aOptions.transparent) {
        var widthPlus20 = width + 20
        return templify('<svg width="{{svgWidth}}" height="{{svgHeight}}" viewBox="-10 -10 {{widthPlus20}} 220" xmlns="http://www.w3.org/2000/svg"{{{xlink}}} role="img" aria-label="{{accessibleText}}"><title>{{{accessibleText}}}</title><g><rect fill="none" stroke="#{{borderColor}}" stroke-width="12" {{borderDashAttr}} rx="100" width="{{width}}" height="200"/></g><g aria-hidden="true" text-anchor="start" font-family="Verdana,DejaVu Sans,sans-serif" font-size="110">{{{labelShadowTag}}}<text x="{{sbTextStart}}" y="138" textLength="{{sbTextWidth}}" fill="#{{labelTextColor}}">{{{label}}}</text>{{{statusShadowTag}}}<text x="{{sbRectWidth45}}" y="138" textLength="{{stTextWidth}}" fill="#{{textColor}}">{{{status}}}</text></g>{{{icon}}}</svg>', {
          svgWidth: aOptions.scale * widthPlus20 / 10,
          svgHeight: aOptions.scale * 22,
          width: width,
          widthPlus20: widthPlus20,
      ...
      var widthPlus20 = width + 20
      return templify('<svg width="{{svgWidth}}" height="{{svgHeight}}" viewBox="-10 -10 {{widthPlus20}} 220" xmlns="http://www.w3.org/2000/svg"{{{xlink}}} role="img" aria-label="{{accessibleText}}"><title>{{{accessibleText}}}</title><g><rect fill="#{{labelColor}}" width="{{sbRectWidth}}" height="200" rx="100"/><rect fill="#{{color}}" x="{{sbRectWidth}}" width="{{stRectWidth}}" height="200" rx="100"/><rect fill="none" stroke="#{{borderColor}}" stroke-width="12" {{borderDashAttr}} rx="100" width="{{width}}" height="200"/></g><g aria-hidden="true" fill="#fff" text-anchor="start" font-family="Verdana,DejaVu Sans,sans-serif" font-size="110">{{{labelShadowTag}}}<text x="{{sbTextStart}}" y="138" textLength="{{sbTextWidth}}" fill="#{{labelTextColor}}">{{{label}}}</text>{{{statusShadowTag}}}<text x="{{sbRectWidth45}}" y="138" textLength="{{stTextWidth}}" fill="#{{textColor}}">{{{status}}}</text></g>{{{icon}}}</svg>', {
        svgWidth: aOptions.scale * widthPlus20 / 10,
        svgHeight: aOptions.scale * 22,
        width: width,
        widthPlus20: widthPlus20,
    ...
    if (aOptions.transparent) {
      var widthPlus20 = width + 20
      return templify('<svg width="{{svgWidth}}" height="{{svgHeight}}" viewBox="-10 -10 {{widthPlus20}} 220" xmlns="http://www.w3.org/2000/svg"{{{xlink}}} role="img" aria-label="{{accessibleText}}"><title>{{{accessibleText}}}</title><g><rect width="{{width}}" height="200" rx="30" fill="none" stroke="#{{borderColor}}" stroke-width="12" {{borderDashAttr}}/></g><g aria-hidden="true" text-anchor="start" font-family="Verdana,DejaVu Sans,sans-serif" font-size="110">{{{labelShadowTag}}}<text x="{{sbTextStart}}" y="138" textLength="{{sbTextWidth}}" fill="#{{labelTextColor}}">{{{label}}}</text>{{{statusShadowTag}}}<text x="{{sbRectWidth45}}" y="138" textLength="{{stTextWidth}}" fill="#{{textColor}}">{{{status}}}</text></g>{{{icon}}}</svg>', {
        svgWidth: aOptions.scale * widthPlus20 / 10,
        svgHeight: aOptions.scale * 22,
        width: width,
        widthPlus20: widthPlus20,
    ...
    var widthPlus20 = width + 20
    return templify('<svg width="{{svgWidth}}" height="{{svgHeight}}" viewBox="-10 -10 {{widthPlus20}} 220" xmlns="http://www.w3.org/2000/svg"{{{xlink}}} role="img" aria-label="{{accessibleText}}"><title>{{{accessibleText}}}</title><linearGradient id="a" x2="0" y2="100%"><stop offset="0" stop-opacity=".1" stop-color="#EEE"/><stop offset="1" stop-opacity=".1"/></linearGradient><mask id="m"><rect width="{{width}}" height="200" rx="30" fill="#FFF"/></mask><g mask="url(#m)"><rect width="{{sbRectWidth}}" height="200" fill="#{{labelColor}}"/><rect width="{{stRectWidth}}" height="200" fill="#{{color}}" x="{{sbRectWidth}}"/><rect width="{{width}}" height="200" fill="url(#a)"/></g><g aria-hidden="true" fill="#fff" text-anchor="start" font-family="Verdana,DejaVu Sans,sans-serif" font-size="110">{{{labelShadowTag}}}<text x="{{sbTextStart}}" y="138" textLength="{{sbTextWidth}}" fill="#{{labelTextColor}}">{{{label}}}</text>{{{statusShadowTag}}}<text x="{{sbRectWidth45}}" y="138" textLength="{{stTextWidth}}" fill="#{{textColor}}">{{{status}}}</text></g>{{{icon}}}</svg>', {
      svgWidth: aOptions.scale * widthPlus20 / 10,
      svgHeight: aOptions.scale * 22,
      width: width,
      widthPlus20: widthPlus20,
